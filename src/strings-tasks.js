...
function removeFirstOccurrences(str, value) {
  if (!value || !str.includes(value)) return str;
...
function formatTime(minutes, seconds) {
  const a = minutes.toString().padStart(2, '0');
...
function countVowels(str) {
  ...
  for (; i < str.length; i += 1) {
    if ('aeiouyAEIOUY'.includes(str[i])) sum += 1;
  }
  return sum;
}
...
function findLongestWord(sentence) {
  ...
  let temp = '';
  const regExp = /\*|,|\.|\?|!|\$/g;
  const s = sentence.replace(regExp, '');
...
function reverseWords(s) {
  ...
  for (let i = 0; i < s.length; i += 1) {
    if (' ,.!?'.includes(s[i])) {
      answer = `${answer}${temp}${s[i]}`;
      temp = '';
    } else temp = `${s[i]}${temp}`;
    if (i === s.length - 1) answer = `${answer}${temp}`;
  }
  return answer;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if ((index >= 65 && index <= 77) || (index >= 97 && index <= 109)) {
      answer += String.fromCharCode(index + 13);
      chek = false;
    }
...
function getCardId(value) {
  let a = 'A234567891JQK'.indexOf(value[0]);
  a += '♣♦♥♠'.indexOf(value[value.length - 1]) * 13;
  return a;
}
